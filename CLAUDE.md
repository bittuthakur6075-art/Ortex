...
* **Key Features**:
  * **Design language**: Metronic 9 Demo 1 (Tailwind/React) — white canvas, zinc neutrals, blue-500 primary, 280px sidebar + 70px header with breadcrumb, 12px cards with `CardHeader`/`CardFooter`, 34px controls with 6px corners, gridded tables via the `mt-head`/`mt-body` classes in `index.css`. Tokens live in `src/index.css`; the kit in `components/ui/Ui.jsx`.
