...
Security lives in the database (RLS, security-definer RPCs, triggers). A client-side role check is the second line of defence, never the only one.

## Database migration status

**Every migration up to 0048 is applied in production** (checked with `supabase migration list` on 2026-09-26; 0041 and 0044 had been missed until 2026-09-23 and went in with 0045). Check with `supabase migration list` before assuming; `db push` skips a migration already recorded even if its objects were later deleted (this happened with the `product-images` bucket).
