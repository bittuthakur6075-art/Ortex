...
//
// MIRROR of Ortex.Admin/src/lib/anuReply.js (edit both); test/anuIntent.test.mjs
// checks the two write the same words.

/* eslint-disable @typescript-eslint/no-explicit-any */
type R = Record<string, any>

const plural = (n: number, one: string, many = `${one}s`) => `${n} ${n === 1 ? one : many}`
const lines = (...parts: unknown[]): string => parts.flat().filter((p: unknown) => p !== null && p !== undefined && p !== "").join("\n")
...
export function greetReply(firstName: string) {
  return lines(
...
export function unknownReply(query?: string) {
  return lines(
...
export function briefingReply(r: R) {
  ...
  if (r.quotations) {
    ...
    if (exp.length) out.push(`Expiring in 3 days: ${exp.map((x: R) => `${x.number} (${x.customer}, ${x.value})`).join("; ")}.`)
    if (lapsed.length) out.push(`Expired but still marked sent: ${lapsed.map((x: R) => x.number).join(", ")}. Follow up or mark them expired.`)
    if (waiting.length) out.push(`Waiting a week or more for a decision: ${waiting.map((x: R) => x.number).join(", ")}.`)
...
export function customersReply(r: R, query: string) {
  if (r?.ok === false) return r.error || NOT_ALLOWED
  ...
  return lines(
    ...
    bullets(r.results.map((c: R) =>
      `${c.name}${c.company !== "none" ? `, ${c.company}` : ""}, ${c.phone}. ${plural(c.quotations, "quotation")}, won ${c.won_value}. Last: ${c.last_quotation}.`)),
  )
}

export function enquiriesReply(r: R, { query, status, days }: { query?: string; status?: string | null; days?: number | null } = {}) {
  if (r?.ok === false) return r.error || NOT_ALLOWED
...
export function quotationsReply(r: R, { query, status }: { query?: string; status?: string | null } = {}) {
  if (r?.ok === false) return r.error || NOT_ALLOWED
  ...
  return lines(
    ...
    bullets(r.results.map((q: R) => `${q.number}, ${q.customer}, ${q.value}, ${q.status}, valid till ${q.valid_until}`)),
  )
}

export function quotationDetailReply(d: R) {
  ...
  return lines(
    ...
    bullets(d.lines.map((l: R) => `${l.item}: ${l.quantity} at ${l.rate} + ${l.gst} GST`)),
...
export function salesReply(r: R) {
  if (r?.ok === false) return r.error || NOT_ALLOWED
  const out = []
...
export function helpReply(res: R) {
  if (!res.articles?.length) {
    return lines("I don't have a guide for that yet. I can explain:", bullets(res.other_topics || []))
  }
  return lines(res.articles.map((a: R, i: number) => lines(`${i ? "\n" : ""}${a.title}:`, a.steps.map((s: string, n: number) => `${n + 1}. ${s}`))))
}

export function whatsNewReply(res: R) {
  const r = res.releases?.[0]
  if (!r) return "Nothing new has been announced."
  return lines(
    `Latest update${r.version ? ` (version ${r.version}, ${r.date})` : ""}: ${r.title}.`,
    r.summary,
    bullets(r.items.map((i: string) => i.replace(/^(\w+): /, (_: string, k: string) => `${k[0].toUpperCase()}${k.slice(1)}: `))),
  )
}

export function sendConfirmText(teamTitle: string, body: string) {
