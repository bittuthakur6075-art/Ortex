...
* **Signing**: `android/app/ortex-release.keystore` (gitignored) + four `ORTEX_*` properties in `~/.gradle/gradle.properties`; without all four the release silently signs with the debug key. **Current key SHA-256 `65:0D:1A:7D:B9:E6:7E:5D:27:7C:6D:04:89:F9:B5:E6:AD:68:A1:0E:4A:C7:DB:A2:32:13:5B:B2:E7:2E:14:9D`, created 2026-09-23 on the NEW build PC, which is the only PC releases are built on from now (owner, 2026-09-23).** The previous key (`A1:81:74:…`, 1.3.0 to 1.3.2) was lost with the old PC, so 1.4.0 was published as a REQUIRED update whose notes tell every phone to uninstall and reinstall once. A backup is in `Documents/Ortex-signing` (with `gradle.properties.txt`); **copy it off this PC** (password manager or company drive). Verify an APK with `apksigner verify --print-certs`.
...
* **Tabs** (gated by the console's `canAccess`): Home, Chat, Quotes, Leads, Catalogue (route `Products`), Customers (route `Contacts`), plus the notification bell and the Anu button on every tab bar. Invoices, payments, settings and user administration stay in the console.
...
* **Chat tab** (`features/chat/`, tab `Chat`, module `chat` = always; stack route `ChatThread`): Team chat on the phone, the same rows and RPCs as the console (Admin migrations 0045/0046). `lib/chat.ts` (RPCs, photo upload as base64, one realtime channel `ortex-chat-mobile`), `features/chat/useChat.ts` (shared inbox store, `useChatThread` with optimistic sends; reset on sign-out), `ChatScreen` (list, Anu pinned, team channels), `ChatThreadScreen` (inverted list, long-press Reply / Copy / Delete for everyone, photos, AnuScreen's measured keyboard lift), `NewChatSheet`. **Anu in Chat uses no model**: `domain/anuIntent.ts` + `domain/anuReply.ts` are mirrors of the console's (test/anuIntent.test.mjs asserts identical parsing and wording, plus `domain/chat.ts` against `lib/chat.js`); `features/chat/anuEngine.ts` answers from collectionStore through `domain/anu.ts` and the database report RPCs; a team message waits for Send. The tab dot comes from the chat inbox in `OneUiTabBar`. No online/typing presence and no push for chat on the phone yet.
