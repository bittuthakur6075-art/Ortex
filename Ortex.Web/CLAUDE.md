...
* **Data**: no server of its own. `lib/supabaseClient.js` reads the catalogue/work photos and inserts leads. The site reads the `products_public` / `categories_public` views (Admin migration 0020), never the tables, so price, cost, HSN and GST never reach it. The quote wizard and contact form queue to `localStorage` when offline (`lib/leads.js`); an insert that stalls 20 s counts as failed and goes to the outbox, `getTrackingIds()` works with storage blocked, and both submit handlers release the button in `finally`, so a lead is never lost to a hung spinner. The database caps an anonymous enquiry at 32 KB (name 200, email 254, phone 40, message 8000 characters).
...
Gemini Live via `@google/genai`, with a single-use token minted by the `orty-live-token` edge function, rate-limited to 6 per visitor per 10 minutes and 400 a day (Admin migration 0050, `_shared/guard.ts`); past that the widget shows the function's "busy" message.
