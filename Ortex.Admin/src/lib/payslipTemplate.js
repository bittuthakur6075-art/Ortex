// The payslip, laid out as Zoho Payroll's standard template. MIRRORED line for
// line from Ortex.Mobile/src/documents/payslipTemplate.ts (generated with tsc,
// parity-checked by Ortex.Mobile/test/payslipTemplate.test.mjs): edit that file
// and regenerate, never this one. Rendered by components/documents/PayslipSheet.jsx.
const esc = (v) =>
  String(v ?? "")
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
...
`
const kv = (k, v) => `<span class="zp-k">${esc(k)}</span><span class="zp-k">:</span><span class="zp-v">${esc(v)}</span>`
function table(title, rows, totalLabel, total, withYtd, height) {
  const cells = (r) =>
    `<td>${esc(r.name)}</td><td>${esc(inr(r.amount))}</td>${
      withYtd ? `<td class="zp-ytd">${r.ytd == null ? "-" : esc(inr(r.ytd))}</td>` : ""
    }`
...
export function payslipBody(p) {
  ...
  <div class="zp-words"><span class="zp-k">Amount In Words : </span><span class="zp-v">${esc(
    amountInWords(p.netPay)
  )}</span></div>
