...
//
// Mirrored by Ortex.Mobile/src/domain/anuIntent.ts. Edit both.
...
export function teamOf(text) {
  const t = ` ${norm(text)} `
  for (const alias of TEAM_WORDS) {
    if (t.includes(` ${alias} `)) return Object.keys(TEAM_ALIASES).find((k) => TEAM_ALIASES[k].includes(alias))
  }
  return null
}
...
export function queryOf(text, extra = []) {
