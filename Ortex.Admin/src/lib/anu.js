// Anu for staff: the pure answers behind her tools, in the console.
//
// A port of Ortex.Mobile/src/domain/anu.ts (edit both sides). The website's
// Anu sells to a stranger; this one works FOR the signed-in colleague. Every
// function here is a plain function of rows the console already reads under
// that person's own Supabase session, so what Anu can say is exactly what RLS
// lets them see, and nothing in this file can widen it.
//
...
// Tested in anu.test.js.

import { formatDate } from "./format"
import { ENQUIRY_STATUS, QUOTATION_STATUS, newLine, statusMeta } from "../data/domain/schema"
import { VOICE_SOURCE, parseQuantity, voiceCallsFrom } from "../pages/voice-leads/helpers"
...
function ranked(items, query, fields) {
  return items
...
const daysAgo = (ts, now) => Math.max(0, Math.floor((now - time(ts)) / DAY_MS))

/** "today", "yesterday", "5 days ago": how a person says it on a call. */
export function spokenAge(ts, now) {
  if (!time(ts)) return "an unknown date"
  const d = daysAgo(ts, now)
  return d === 0 ? "today" : d === 1 ? "yesterday" : `${d} days ago`
}

const who = (c) => c?.company || c?.name || "an unnamed customer"
// Whole rupees: paise read aloud ("forty-eight thousand rupees and zero paise")
// is noise on a call, and every figure here is already rounded on its document.
export const money = (n) => `₹${Math.round(Number(n) || 0).toLocaleString("en-IN")}`
const isWon = (s) => s === "accepted" || s === "invoiced"

// ---- briefing ----------------------------------------------------------------

// "What needs me today?" The same signals the notification drawer raises, as
// one spoken-length answer: untouched web enquiries, Anu's calls nobody has
// rung back, sent quotations about to lapse or already lapsed, and quotations
// waiting on a decision for a week.
export function briefing(data, access, now = Date.now()) {
  const out = {}
  ...
  if (access.quotations) {
    ...
    const until = (q) => time(q.validUntil)
...
// The briefing as counts, for the panel's "For you" suggestions: each one is a
// question worth asking only when there is something behind it.
export function attentionCounts(data, access, now = Date.now()) {
  const b = briefing(data, access, now)
  return {
...
export function findCustomers(customers, quotations, query) {
...
export function findEnquiries(enquiries, opts, access, now = Date.now()) {
  const since = opts.days ? now - opts.days * DAY_MS : 0
  const statusOk = (s) => !opts.status || (s || "new") === opts.status
...
export function findQuotations(quotations, opts) {
  const hits = ranked(
...
export function quotationDetail(q) {
  return {
    id: q.id,
    kind: "quotation",
    number: q.number,
...
export function salesSummary(data, days, access, now = Date.now()) {
  const since = now - days * DAY_MS
  const out = { period: `the last ${days} days` }
  if (access.quotations) {
...
export function draftLines(products, items) {
  const unmatched = []
  const lines = (items || [])
...
function findProductRows(products, query) {
  return ranked(
...
// A record a tool result names, as the card the panel draws under Anu's answer.
export function cardFor(r) {
...
// Where a record lives in the console, as a react-router target.
export function routeFor(kind, id) {
  switch (kind) {
    case "customer": return { to: `/customers/${id}` }
    case "enquiry": return { to: `/enquiries/${id}` }
    case "voice_call": return { to: "/crm?tab=voice", state: { openId: id } }
    case "quotation": return { to: "/quotations", state: { openId: id } }
    case "product": return { to: "/catalog?tab=products", state: { openId: id } }
    default: return null
  }
}
