...
/** The headline numbers over jobs and calls. Shared with the Dashboard's Call agent card. */
export function telecallerStats(jobs, calls, now = Date.now()) {
  const dayStart = new Date(now).setHours(0, 0, 0, 0)
  const week = calls.filter((c) => new Date(c.createdAt).getTime() >= now - 7 * DAY_MS)
  ...
  const due = jobs.filter((j) => j.status === "queued" && new Date(j.scheduledAt || 0).getTime() <= now).length
  const upcoming = jobs.filter((j) => j.status === "queued" && new Date(j.scheduledAt || 0).getTime() > now).length
  const inFlight = jobs.filter((j) => ["dialing", "in_progress"].includes(j.status)).length
  const today = calls.filter((c) => new Date(c.createdAt).getTime() >= dayStart).length
  const actions = calls.filter((c) => c.status === "completed" && ACTION_OUTCOMES.has(c.analysis?.outcome) && !c.handled).length
...
export function useTelecallerData() {
  ...
  const stats = useMemo(() => telecallerStats(jobs.items, calls.items), [jobs.items, calls.items])
